            ...
            <template #header>
              <div class="row items-center justify-between full-width q-pa-sm">
                ...
                <div class="column items-end">
                  ...
                  <div v-if="results[t.id]?.valid" class="text-caption">
                    정가 EV: <b>{{ formatMoney(results[t.id].ev) }}</b>
                  </div>
                </div>
              </div>
            </template>
...
function headerSubtitle(t) {
  const modeLabel = MODE_LABEL[t.mode] || '모드 미설정'
  const entries = t.entries ? `${t.entries} 엔트리 예상` : '엔트리 미입력'
  return `${modeLabel} · ${entries}`
}
...
/* ============================================================
   비교 테이블 columns (모바일/데스크톱 분리)
============================================================ */

const tableColsMobile = [
  { name: 'label', label: '대회', field: 'label', align: 'left' },
  { name: 'ev', label: '정가 EV', field: 'ev', align: 'right' },
  { name: 'evDiscount', label: '할인 EV', field: 'evDiscount', align: 'right' },
  { name: 'grade', label: '등급', field: 'grade', align: 'center' },
]

const tableColsDesktop = [
  { name: 'label', label: '대회', field: 'label', align: 'left' },
...
const tableCols = computed(() => ($q.screen.lt.md ? tableColsMobile : tableColsDesktop))
...
/* expansion-item 좌측 패딩 최소화 */
.ev-expansion .q-expansion-item__header {
